...
const StudentForm = ({ onSubmit, onTemplateChange, initialData }) => {
  ...
  // Watch for template changes to update live preview
  const templateValue = watch("template");
  
  useEffect(() => {
    onTemplateChange(templateValue);
  }, [templateValue, onTemplateChange]);
  ...
  const handlePhotoChange = (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      alert("File is too large! Please select an image less than 2MB.");
      return;
    }
  ...
  return (
    <Card>
      <CardContent className="pt-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-2 border-b">Student Information</h2>
